...
const store = new Vuex.Store({
    ...
    mutations: {
        changeTopRow(state, {abbr}) {
            const listAbbr = state.listAbbr;
        ...
        toggleEditing(state, {abbr}) {
            // 如果传值为空，退出编辑
            if (!abbr) {
                state.editingAbbr = null;
                return;
            }

            // 如果与当前编辑相同
            // 切换编辑状态 (abbr|null)
            if (abbr === state.editingAbbr) {
                state.editingAbbr = state.editingAbbr ? null : abbr;
                return;
            }

            // 如果不同
            state.editingAbbr = abbr;
        }
    },
    actions: {},
    getters: {
        ...
        // 仅用于显示
        getAmount: ({listAbbr, amount}, getter) => ({abbr}) => {
